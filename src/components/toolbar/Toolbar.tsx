...
const Toolbar: React.FC = () => {
  ...
  const selectedObject = useMemo(() => {
    return textObjects.find(obj => obj.id === selectedObjectId) ||
           imageObjects.find(obj => obj.id === selectedObjectId);
    ...
    <div className="w-full h-full p-4 flex flex-col gap-4 overflow-y-auto">
      ...
      <div className="flex flex-col gap-2">
        <h3 className="text-sm font-semibold text-gray-600">
          객체 {selectedObject ? `(${selectedObject.hasCheckbox ? '체크박스' : '텍스트'})` : 
                selectedObject ? '(이미지)' : ''}
        </h3>
        ...
        {selectedObject ? (
          <>
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <div className="mb-3">
                ...
                <div className="mb-2">
                  ...
                  <div className="flex items-center gap-2">
                    <input 
                      ...
                      className="flex-1" 
                    />
                    <span className="text-xs text-gray-600 w-8">{selectedObject.fontSize || 16}px</span>
                  </div>
                </div>
                ...
                <div className="mb-2">
                  ...
                  <select 
                    value={selectedObject.textStyle?.fontFamily || 'Arial, sans-serif'}
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  ...
                  <div className="flex gap-1">
                    <button 
                      onClick={() => debouncedUpdateTextStyle({ bold: !selectedObject.textStyle?.bold })}
                    ...
                    <button 
                      onClick={() => debouncedUpdateTextStyle({ italic: !selectedObject.textStyle?.italic })}
                      className={`flex-1 px-2 py-1 border rounded text-xs transition-colors ${
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 mb-2">
                  <input 
                    type="color" 
                    value={selectedObject.boxStyle?.backgroundColor || '#ffffff'}
                  ...
                  <input 
                    type="color" 
                    value={selectedObject.boxStyle?.borderColor || '#000000'}
                ...
                <div className="grid grid-cols-2 gap-2">
                  <div>
                    <label className="text-xs text-gray-500">배경투명도</label>
                    <input 
                      type="range" 
                      min="0" 
                      max="100" 
                      value={(selectedObject.boxStyle?.backgroundOpacity || 1) * 100}
                      onChange={(e) => debouncedUpdateBoxStyle({ backgroundOpacity: parseInt(e.target.value) / 100 })}
                      className="w-full" 
                    />
                  </div>
                  <div>
                    <label className="text-xs text-gray-500">테두리 굵기</label>
                    <input 
                      type="range" 
                      min="0" 
                      max="10" 
                      value={selectedObject.boxStyle?.borderWidth || 0}
                ...
                <div className="grid grid-cols-1 gap-2 mt-2">
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => debouncedUpdateTextObject(selectedObject.id, { opacity: parseInt(e.target.value) / 100 })}
                      className="w-full" 
                    />
                  </div>
                </div>
            ...
            {/* 체크박스 색상 (체크박스가 있는 경우에만) */}
            {selectedObject.hasCheckbox && (
              <div className="bg-gray-50 rounded-lg p-3">
                <h4 className="text-xs font-semibold text-gray-500 mb-2">체크박스 색상</h4>
                <div className="space-y-2">
                  <div>
                    <label className="text-xs text-gray-600 mb-1 block">체크된 색상</label>
                    <input 
                      type="color" 
                      value={selectedObject.checkboxCheckedColor || defaultCheckedColor}
                  ...
                  <div>
                    ...
                    <input 
                      type="color" 
                      value={selectedObject.checkboxUncheckedColor || defaultUncheckedColor}
            ...
            <div className="bg-gray-50 rounded-lg p-3">
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.movable || false}
                    onChange={(e) => debouncedUpdatePermissions({ movable: e.target.checked })}
                    className="rounded" 
                  />
                  <span>이동가능</span>
                </label>
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.deletable || false}
                    onChange={(e) => debouncedUpdatePermissions({ deletable: e.target.checked })}
                    className="rounded" 
                  />
                  <span>삭제가능</span>
                </label>
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.resizable || false}
                    onChange={(e) => debouncedUpdatePermissions({ resizable: e.target.checked })}
                    className="rounded" 
                  />
                  <span>크기조절 가능</span>
                </label>
              ...
              <div className="grid grid-cols-1 gap-2">
                <div>
                  ...
                  <input 
                    ...
                    max="100" 
                    value={(selectedObject.opacity || 1) * 100}
                    onChange={(e) => debouncedUpdateImageObject(selectedObject.id, { opacity: parseInt(e.target.value) / 100 })}
                    className="w-full" 
                  />
                </div>
                ...
                <div>
                  <label className="flex items-center gap-2 text-xs">
                    <input 
                      ...
                      onChange={(e) => debouncedUpdateImageObject(selectedObject.id, { maintainAspectRatio: e.target.checked })}
                      className="rounded" 
                    />
                    <span>원본 비율 유지</span>
                  </label>
              ...
              <div className="flex flex-col gap-2">
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.movable || false}
                    onChange={(e) => debouncedUpdateImageObject(selectedObject.id, { permissions: { ...selectedObject.permissions, movable: e.target.checked } })}
                    className="rounded" 
                  />
                  <span>이동가능</span>
                </label>
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.deletable || false}
                    onChange={(e) => debouncedUpdateImageObject(selectedObject.id, { permissions: { ...selectedObject.permissions, deletable: e.target.checked } })}
                    className="rounded" 
                  />
                  <span>삭제가능</span>
                </label>
                <label className="flex items-center gap-2 text-xs">
                  <input 
                    type="checkbox" 
                    checked={selectedObject.permissions?.resizable || false}
                    onChange={(e) => debouncedUpdateImageObject(selectedObject.id, { permissions: { ...selectedObject.permissions, resizable: e.target.checked } })}
                    className="rounded" 
                  />
                  <span>크기조절 가능</span>
                </label>
      ...
      <div className="mt-auto p-3 bg-gray-50 rounded-lg">
        ...
        {selectedObject && (
          <div className="text-xs text-gray-500 mt-1">
            선택된 객체: {selectedObject.hasCheckbox ? '체크박스' : '텍스트'}
          </div>
        )}
      </div>
    </div>
